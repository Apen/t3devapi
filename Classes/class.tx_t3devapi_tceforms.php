...
class tx_t3devapi_tceforms
{
    ...
    public function insertRecord($datas)
    {
        ...
        //\TYPO3\CMS\Core\Utility\HttpUtility::redirect($this->pObj->getEditUrl($uid) . '&' . $this->prefixId . '[insertok]=1');
        \TYPO3\CMS\Core\Utility\HttpUtility::redirect($this->pObj->getListUrl());
    }
    ...
    public function hookForInsertRecord($datas)
    {
        if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['recordsmanagerfe']['hookForInsertRecord'])) {
            foreach ($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['recordsmanagerfe']['hookForInsertRecord'] as $classRef) {
                $procObj = &\TYPO3\CMS\Core\Utility\GeneralUtility::getUserObj($classRef);
    ...
    public function checkFieldFromTcaEval($field, $val)
    {
        ...
        if (empty($val) && !\TYPO3\CMS\Core\Utility\GeneralUtility::inList($config['eval'], 'required')) {
            return $val;
        }
        $evalList = explode(',', $config['eval']);
        foreach ($evalList as $eval) {
    ...
    /**
     * Load the javascript of tinymce
     *
     * @return void
     */
    public function loadTinyMce()
    {
        $headerkey = $this->formName . 'js';
