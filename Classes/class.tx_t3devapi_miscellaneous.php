...
class tx_t3devapi_miscellaneous
{
    ...
    {
        \TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($var, $header);
    }
    ...
    public function getURL($additionalParamsArray = array(), $cache = 0, $altPageId = 0)
    {
        ...
        if ($this->pObj->cObj->getUserObjectType() == \TYPO3\CMS\Frontend\ContentObject\ContentObjectRenderer::OBJECTTYPE_USER_INT) {
            $conf['useCacheHash'] = 0;
        }
        $conf['no_cache'] = 0;
        $conf['returnLast'] = 'url';
        $conf['parameter'] = $altPageId ? $altPageId : $GLOBALS['TSFE']->id;
        $conf['additionalParams'] = \TYPO3\CMS\Core\Utility\GeneralUtility::implodeArrayForUrl('', $additionalParamsArray, '', 1);
    ...
    {
        $cmd = \TYPO3\CMS\Core\Utility\GeneralUtility::imageMagickCommand('convert', $params . ' ' . escapeshellarg($input) . ' ' . escapeshellarg($output));
        $ret = exec($cmd);
        \TYPO3\CMS\Core\Utility\GeneralUtility::fixPermissions($output); // Change the permissions of the file
        return $ret;
    }
    ...
    public function getPiVars($exclude = '', $prefix = false)
    {
        ...
        foreach ($this->pObj->piVars as $piVar => $piVarvalue) {
            if (!\TYPO3\CMS\Core\Utility\GeneralUtility::inList($exclude, $piVar)) {
                if ($prefix === true) {
    ...
    {
        $useSwiftMailer = \TYPO3\CMS\Core\Utility\GeneralUtility::compat_version('4.5');
        if ($useSwiftMailer) {
    ...
    public function loadLL($LLFile, $langKey = null, $confLL = null)
    {
        ...
        if (@is_file($LLFile)) {
            $useXliff = \TYPO3\CMS\Core\Utility\GeneralUtility::compat_version('4.6');
            if ($useXliff) {
