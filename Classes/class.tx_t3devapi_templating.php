...
class tx_t3devapi_templating
{
    ...
    {
        $templateAbsPath = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName(trim($templateFile));
    ...
    {
        return ltrim(\TYPO3\CMS\Core\Html\HtmlParser::getSubpart($template, $subpart));
    }
    ...
    protected function processHelpersLLL($templateMarkers, $content, $viewHelperArgumentLists)
    {
        foreach ($viewHelperArgumentLists as $viewHelperArgument) {
            if (empty($this->pObj->conf['locallang'][strtolower($viewHelperArgument)]) !== true) {
                ...
                $content = \TYPO3\CMS\Core\Html\HtmlParser::substituteMarker($content, '###LLL:' . $viewHelperArgument . '###', $value);
            }
        }
        return $content;
    }
        ...
        foreach ($viewHelperArgumentLists as $viewHelperArgument) {
            if (empty($templateMarkers['###' . $viewHelperArgument . '###']) === true) {
                $content = \TYPO3\CMS\Core\Html\HtmlParser::substituteSubpart(
                    ...
                    \TYPO3\CMS\Core\Html\HtmlParser::getSubpart($content, '###EMPTY:' . $viewHelperArgument . '###')
                );
            } else {
                $content = \TYPO3\CMS\Core\Html\HtmlParser::substituteSubpart($content, '###EMPTY:' . $viewHelperArgument . '###', '');
            }
        }
        return $content;
    }
        ...
        foreach ($viewHelperArgumentLists as $viewHelperArgument) {
            if (empty($templateMarkers['###' . $viewHelperArgument . '###']) !== true) {
                $content = \TYPO3\CMS\Core\Html\HtmlParser::substituteSubpart(
                    ...
                    \TYPO3\CMS\Core\Html\HtmlParser::getSubpart($content, '###NOTEMPTY:' . $viewHelperArgument . '###')
                );
            } else {
                $content = \TYPO3\CMS\Core\Html\HtmlParser::substituteSubpart($content, '###NOTEMPTY:' . $viewHelperArgument . '###', '');
            }
        }
        return $content;
    }
        ...
        foreach ($viewHelperArgumentLists as $viewHelperArgument) {
            ...
            if ($this->evaluateCondition($comparand1, $comparand2, $operator)) {
                $content = \TYPO3\CMS\Core\Html\HtmlParser::substituteSubpart(
                    ...
                    \TYPO3\CMS\Core\Html\HtmlParser::getSubpart($content, '###IF:' . $viewHelperArgument . '###')
                );
            } else {
    ...
    protected function processHelpersLink($templateMarkers, $content, $viewHelperArgumentLists)
    {
        foreach ($viewHelperArgumentLists as $viewHelperArgument) {
            ...
            $additionalParameters = $explodeViewHelperArguments[1] ?
                \TYPO3\CMS\Core\Utility\GeneralUtility::explodeUrl2Array($explodeViewHelperArguments[1])
                : array();
